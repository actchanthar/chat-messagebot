...
async def rmamount(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Check for arguments (user_id to reset, or none for all users)
    args = context.args
    if args and len(args) == 1:
        ...
        # Reset withdrawn_today for the specific user
        try:
            await db.update_user(target_user_id, {
                "withdrawn_today": 0,
                "last_withdrawal": None  # Optionally reset last_withdrawal to allow immediate new withdrawals
            })
        ...
        # Reset withdrawn_today for all users
