...
async def dfusers(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    logger.info(f"/dfusers command initiated by user {user_id} in chat {chat_id}")

    if user_id not in ADMIN_IDS:
        await update.message.reply_text("Only admins can delete failed broadcast users.")
        logger.info(f"User {user_id} attempted /dfusers but is not an admin")
        return
    ...
    try:
        ...
        logger.error(f"Failed to send /dfusers response to user {user_id}: {e}")
        try:
            await context.bot.send_message(
                chat_id=LOG_CHANNEL_ID,
                text=f"Failed to send /dfusers response to {user_id}: {e}"
            )
        except Exception as log_error:
            logger.error(f"Failed to log /dfusers error to {LOG_CHANNEL_ID}: {log_error}")
